...
xhrequest.open('GET', 'products.json');
xhrequest.responseType = 'json';
xhrequest.onload = function () {
  if(xhrequest.status === 200) {
    let products = xhrequest.response;
    initialize(products);
  } else {
    console.log('XHR problem: ' + xhrequest.status + ' => ' + xhrequest.statusText);
  }
...
function initialize(products) {
  ...
  function fetchBlob(product) {
    ...
    xhrequest.onload = () => {
      if (xhrequest.status === 200) {
        let objectURL = URL.createObjectURL(xhrequest.response);
        showProduct(objectURL, product);
      } else {
        console.log('Network request for "' + product.name + '" image failed with response ' + xhrequest.status + ': ' + xhrequest.statusText);
      }
    };
    xhrequest.send();
  }
